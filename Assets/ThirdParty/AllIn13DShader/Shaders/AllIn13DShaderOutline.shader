Shader "AllIn13DShader/AllIn13DShaderOutline"
{
	Properties
	{
		...
		[EffectProperty(ParentEffect# SHADINGMODEL, Keywords(_SHADINGMODEL_PBR), AllowReset# True)][Toggle(_METALLIC_MAP_ON)]_MetallicMapOn("Use Metallic Map?", Float) = 0.0
		...
		[EffectProperty(ParentEffect# SHADINGMODEL, Keywords(_SHADINGMODEL_PBR), AllowReset# True)][Toggle(_GLOSS_MAP_ON)]_GlossMapOn("Use Gloss Map?", Float) = 0.0
		[EffectProperty(ParentEffect# SHADINGMODEL, KeywordsOp# AND, Keywords(_SHADINGMODEL_PBR, _GLOSS_MAP_ON), AllowReset# True)]_GlossMap("Gloss Map", 2D) = "white" {}
		...
		*/
		...
		[EffectProperty(DITHER)]_DitherScale("Scale", Range(1, 20)) = 1
